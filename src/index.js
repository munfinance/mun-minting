...
const App = () => {
	...
	const setupContracts = async (munAddress, musAddress, _nftManager) => {
		const mun = new window.web3.eth.Contract(
			config.munAbi,
			munAddress,
		)
		const mus = new window.web3.eth.Contract(
			config.musAbi,
			musAddress,
		)
	...
	const getCards = async () => {
		...
		let combined = await Promise.all(uris.map(async (uri, i) => {
			...
			return {
				...
				requiredMUN: window.web3.utils.fromWei(blueprint[2]),
				requiredMUS: window.web3.utils.fromWei(blueprint[3]),
